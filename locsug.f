     ...
     1                  ug ,vg ,u10,u20,v10,v20,r,z,dist,imes)
      parameter(eps1=0.001,eps2=0.00001,eps3=0.001)
      parameter(mdimn=5)
      parameter(miter=100)
c
...
c
      ...
      h12  = h(1)*h(2)
      do 50 id=1,ndimn
       xden = xden + h11*ru(id)*ru(id)+2*h12*ru(id)*rv(id)+
     1               h22*rv(id)*rv(id)
       xpp  = xpp  +(h11*ruu(id)+h22*rvv(id)+2*h12*ruv(id))*
     1               z(id)
50    continue
      if(xpp+xden.gt.eps3*xl*xl)xden = xden+xpp
      da = -0.5*(xgr1*h(1)+xgr2*h(2))/xden
c
c limit step
c
